...
const NegitveQuantity = async ({product_id}) => {
try {
        let accessToken = JSON.parse(localStorage.getItem("user"))?.accessToken;

        let res = await fetch("http://localhost:5000/api/cart/negitave", {
            method: "POST",
            headers: {
        ...
        if (res.status === 401) {
            ...
            if (RefreshRes.ok) {
                ...
                res = await fetch("http://localhost:5000/api/cart/negitave", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Accept": "application/json",
        ...
        if (!res.ok) {
            return { success: false, msg: data.message || data.msg || "Something went wrong" };
        } else {
                return { success: true, msg: "successfully", data };
            }
        }
...
export default NegitveQuantity